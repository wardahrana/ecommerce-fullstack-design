...
import LoginModal from '../shared/LoginModal';  // ← Fixed path
...
export default function AppHeader() {
    ...
    const handleProfileClick = () => {
        if (activeUser) {
            ...
            setLoginOpen(true);  // ← Open login modal instead of signup
        }
    };
    ...
    return (
        <>
                    ...
                    <div className="max-w-screen-xl mx-auto px-4">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-6 py-2">
                                <button className="flex items-center gap-2 text-sm text-gray-700 hover:text-blue-500 transition-colors font-medium">
                                ...
                                {NAV_LINKS.slice(1).map((link) => (
                                    <Link
                                        key={link.label}
                                        to={link.path}
                                        className="text-sm text-gray-600 hover:text-blue-500 transition-colors whitespace-nowrap relative group"
                                    >
                                        {link.label}
                                        <span className="absolute -bottom-0.5 left-0 w-0 h-0.5 bg-blue-500 rounded transition-all duration-200 group-hover:w-full" />
                                    </Link>
                                ))}
                            ...
                            <div className="px-4 py-3 flex flex-col gap-1">
                                {NAV_LINKS.map((link, i) => (
                                    ...
                                    >
                                        <Link
                                            to={link.path}
                                            className="block text-sm text-gray-700 hover:text-blue-500 transition-colors py-2 border-b border-gray-50"
            ...
            {/* ── MODALS (placed outside header, after return) ── */}
            <SignupModal
                isOpen={signupOpen}
